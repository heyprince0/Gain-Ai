...
export async function POST(request: Request) {
  try {
    ...
    if (!member_id || !gym_id || !fcm_token)
      return NextResponse.json({ error: 'member_id, gym_id, and fcm_token are required' }, { status: 400 })

    // Security: member can only register their own token
    if (member_id !== user.id)
      return NextResponse.json({ error: 'Forbidden' }, { status: 403 })

    // ✅ Service role for DB query
