import { supabaseBrowser as supabase } from './supabase-browser' // ← changed
...
export async function getOwnerData(userId: string) {
  ...
  console.log('[getOwnerData] Fetching members for gym:', gym.id)
  ...
  if (membersError) {
    console.error('[getOwnerData] Members error:', membersError)
    throw membersError
  }
  if (plansError) {
    console.error('[getOwnerData] Plans error:', plansError)
    throw plansError
  }

  console.log(`[getOwnerData] Found ${members?.length ?? 0} members, ${plans?.length ?? 0} plans`)
